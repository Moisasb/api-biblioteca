...
export class AuthService {
    ...
    async login(loginDto:LoginDto){
        ...
        // Testamos para ver se usuário existe no banco, caso não exista, retornamos uma mensagem de 'erro'
        if (resultado.length === 0) {
            throw new UnauthorizedException('E-mail ou senha inválidos');
        }
        // Armazena o usuário encontrado dentro da constante 'usuario'
        const usuario = resultado[0];
        // Agora vamos usar o bcrypt.compare para testar se a senha informada pelo usuário para realizar o login, é a mesma senha usada para o cadastro dele
        const senhaValida = await bcrypt.compare(
            loginDto.senha, // senha do login
            usuario.senha // senha do cadastro
        );
        // Caso a senha fornecida não coincida com a senha registrada no cadastramento, retornamos uma mensagem de 'erro'
        if (!senhaValida) {
            throw new UnauthorizedException('E-mail ou senha inválidos');
        }
        // Informações que serão armazenadas dentro do token (sendo neste caso o ID e o E-mail do usuário). Nós não passamos a senha dele pelo token por questões de segurança.
        const payload = {
        ...
        // Se tudo correu bem até aqui, o login será realizado e junto, será gerado e exibido o token
